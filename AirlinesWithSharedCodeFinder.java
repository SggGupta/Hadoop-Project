 ...
 * @version 1.0
 * 
 *  Program to find airlines having shared code. By default it will find  all airlines having
 *  shared code. But this program also takes optional parameter if user want find airlines having
 *  shared code in specific country.
 *  
 *  Usage: airlinesWithSharedCodeFinder <input-routesData> <input-airlines> <output> [country]
 */
public class AirlinesWithSharedCodeFinder extends Configured implements Tool {

	
	public static void main(String[] args) throws Exception{                               
	   
	    int res = ToolRunner.run(new AirlinesWithSharedCodeFinder(), args);
	...
	public int run(String[] allArgs) throws Exception {
	    ...
	    if (otherArgs.length < 3) {
	      System.err.println("Usage: airlinesWithSharedCodeFinder <input> <input> <output> [country]");
	     return -1;
	    }   
		config.set("airlineAppCmd", AirlineCmdTypes.SHARED_CODE_CMD_VALUE+"");
		...
		return airlineJobRunner.invokeService(config, otherArgs,AirlinesWithSharedCodeFinder.class);
